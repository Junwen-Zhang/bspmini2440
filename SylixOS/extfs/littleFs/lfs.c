        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            if (superblock.file_max) {
                if (superblock.file_max > lfs->file_max) {
                    LFS_ERROR("Unsupported file_max (%d > %d)",
            ...
            if (superblock.block_count != lfs->cfg->block_count) {
                LFS_ERROR("Invalid block count (%d != %d)",
            ...
            if (superblock.block_size != lfs->cfg->block_size) {
                LFS_ERROR("Invalid block size (%d != %d)",
                        superblock.block_count, lfs->cfg->block_count);
                err = LFS_ERR_INVAL;
                goto cleanup;
            }
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    LFS_TRACE("lfs_format(%p, %p {.context=%p, "
                ...
                ".read_size=%d, .prog_size=%d, "
                ".block_size=%d, .block_count=%d, "
                ".block_cycles=%d, .cache_size=%d, "
                ".lookahead_size=%d, .read_buffer=%p, "
                ".prog_buffer=%p, .lookahead_buffer=%p, "
                ".name_max=%d, .file_max=%d, "
                ".attr_max=%d})",
...
lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path,
    ...
    LFS_TRACE("lfs_getattr(%p, \"%s\", %"PRIu8", %p, %d)",
...
int lfs_setattr(lfs_t *lfs, const char *path,
    ...
    LFS_TRACE("lfs_setattr(%p, \"%s\", %"PRIu8", %p, %d)",
            (void*)lfs, path, type, buffer, size);
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file, const char *path, int flags) {
    ...
    LFS_TRACE("lfs_file_open(%p, %p, \"%s\", %d)",
...
int lfs_file_opencfg(lfs_t *lfs, lfs_file_t *file,
    ...
    LFS_TRACE("lfs_file_opencfg(%p, %p, \"%s\", %d, %p {"
                 ".buffer=%p, .attrs=%p, .attr_count=%d})",
            (void*)lfs, (void*)file, path, flags,
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file,
    ...
    LFS_TRACE("lfs_file_read(%p, %p, %p, %d)",
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
    ...
    LFS_TRACE("lfs_file_write(%p, %p, %p, %d)",
            (void*)lfs, (void*)file, buffer, size);
    LFS_ASSERT(lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file));
...
int lfs_file_truncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    LFS_TRACE("lfs_file_truncate(%p, %p, %d)",
